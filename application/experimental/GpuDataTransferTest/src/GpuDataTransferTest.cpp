...
template <u_int64_t InputSize, u_int64_t ComputationHeavyIterations, u_int64_t LargeInputSize>
void runParallelSeparateQueues(sycl::queue& computationHeavyQueue, sycl::queue& transferHeavyQueue, unsigned iterations, std::vector<float>& executionTimes);


int main()
{
    sycl::queue queue0(sycl::gpu_selector_v);
    ...
    runSequential<inputSize, computationHeavyIterations, largeInputSize>(queue0, iterations, computationHeavyExecutionTimes, transferHeavyExecutionTimes);

    std::cout << "Sequential execution times:" << std::endl;
    std::cout << "iteration\texecution sum\tcomputation\ttransfer" << std::endl;
    ...
    runParallel<inputSize, computationHeavyIterations, largeInputSize>(queue0, iterations, parallelExecutionTimes);
    ...
    sycl::queue queue1(sycl::gpu_selector_v);

    std::vector<float> executionTimesSeparateQueues;
    runParallelSeparateQueues<inputSize, computationHeavyIterations, largeInputSize>(queue0, queue1, iterations, executionTimesSeparateQueues);

    std::cout << "Parallel execution times with separate queues:" << std::endl;
    std::cout << "iteration\texecution time" << std::endl;
    for (unsigned i = 0; i < 10; ++i)
    {
        std::cout << i << "\t\t" << executionTimesSeparateQueues[i] << std::endl;
    }
}
...
void runParallel(sycl::queue& queue, unsigned iterations, std::vector<float>& executionTimes)
{
    for (unsigned i = 0; i < iterations; ++i)
    {
        ...
        std::chrono::steady_clock::time_point tasksStart;
        std::chrono::steady_clock::time_point tasksEnd;

        tasksStart = std::chrono::steady_clock::now();
        queue.submit([&](sycl::handler& handler) {
            sycl::accessor<float, 1, sycl::access::mode::read, sycl::access::target::device> inputAccessor(inputBuffer, handler, sycl::read_only);
            sycl::accessor<float, 1, sycl::access::mode::write, sycl::access::target::device> outputAccessor(outputBuffer, handler, sycl::write_only);
        ...
        queue.wait();
        tasksEnd = std::chrono::steady_clock::now();

        executionTimes.push_back(std::chrono::duration_cast<std::chrono::milliseconds>(tasksEnd - tasksStart).count());
    }
}
...
void runParallelSeparateQueues(sycl::queue& computationHeavyQueue, sycl::queue& transferHeavyQueue, unsigned iterations, std::vector<float>& executionTimes)
{
    for (unsigned i = 0; i < iterations; ++i)
    {
        ...
        tasksStart = std::chrono::steady_clock::now();
        computationHeavyQueue.submit([&](sycl::handler& handler) {
            sycl::accessor<float, 1, sycl::access::mode::read, sycl::access::target::device> inputAccessor(inputBuffer, handler, sycl::read_only);
            sycl::accessor<float, 1, sycl::access::mode::write, sycl::access::target::device> outputAccessor(outputBuffer, handler, sycl::write_only);
        ...
        transferHeavyQueue.submit([&](sycl::handler& handler) {
            sycl::accessor<float, 1, sycl::access::mode::read, sycl::access::target::device> inputAccessor(largeInputBuffer, handler, sycl::read_only);
            sycl::accessor<float, 1, sycl::access::mode::write, sycl::access::target::device> outputAccessor(largeOutputBuffer, handler, sycl::write_only);
        ...
        computationHeavyQueue.wait();
        transferHeavyQueue.wait();
        tasksEnd = std::chrono::steady_clock::now();
